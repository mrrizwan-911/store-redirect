...
export async function generateQuotationDraft(quotation: any) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return `Dear ${quotation.name},\n\nThank you for your interest in Antigravity Atelier. We have prepared the bulk order quotation you requested for ${quotation.company || 'your company'}.\n\nPlease find the detailed quotation attached to this email. We look forward to the possibility of working together.\n\nBest regards,\nAntigravity Atelier Team`;
  }
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 1000,
      system: 'You are a professional B2B sales representative for Antigravity Atelier, a premium luxury apparel brand. Your goal is to write a concise, elegant, and persuasive cover letter for a bulk order quotation.',
      messages: [
        {
          role: 'user',
          ...
          The tone should be sophisticated, helpful, and luxury-oriented. Mention that the detailed quotation is attached. Keep it under 200 words. Do not use placeholders like [Your Name], just sign off as "Antigravity Atelier Team".`
        }
      ]
    });
    ...
    return `Dear ${quotation.name},\n\nThank you for reaching out to Antigravity Atelier. We are pleased to provide you with the bulk order quotation for ${quotation.company || 'your inquiry'}.\n\nPlease find the details in the attached PDF. Our team is available to discuss any specific requirements or customizations you may need.\n\nBest regards,\nAntigravity Atelier Team`;
  }
}
